...
const RateHawk = () => {
    ...
    const [searchParams, setSearchParams] = useState({
        created_at: {
            from_date: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] + 'T00:00' // Last 30 days
        }
    })
    ...
    const data = {
        ...
        rows: orders.map((order) => ({
            ...
            dates: `${order.checkin_at} to ${order.checkout_at}`,
            ...
            actions: <CButton color="primary" size="sm" onClick={() => handleRowClick(order)}>View Details</CButton>
        }))
    }
...
<CTableRow>
  ...
  <CTableDataCell>{selectedOrder.supplier_data?.confirmation_id || 'N/A'}</CTableDataCell>
</CTableRow>
  ...
  <CTableDataCell>
    {selectedOrder.amount_sell?.amount} {selectedOrder.amount_sell?.currency_code}
  </CTableDataCell>
</CTableRow>
...
<CTableRow>
  ...
  <CTableDataCell>
    {selectedOrder.amount_payable?.amount} {selectedOrder.amount_payable?.currency_code}
  </CTableDataCell>
</CTableRow>
      ...
      : `Due on ${formatDate(selectedOrder.payment_data?.payment_due)}`}
                                ...
                                <CCol>
                                    ...
                                    {selectedOrder.rooms_data?.map((room, index) => (
                                        <CCard key={index} className="mb-3">
                                            <CCardBody>
                                                <h6>Room {index + 1}: {room.room_name}</h6>
                                                <p><strong>Meal:</strong> {room.meal_name}</p>
                                                <p><strong>Bedding:</strong> {room.bedding_name?.join(', ') || 'N/A'}</p>
                                                
                                                <h6 className="mt-2">Guests ({room.guest_data?.adults_number} Adults, {room.guest_data?.children_number} Children)</h6>
                            ...
                            <CRow className="mb-3">
                                <CCol>
                                    ...
                                    <CCard>
                                        <CCardBody>
                                            <p><strong>Cancellable:</strong> {selectedOrder.is_cancellable ? 'Yes' : 'No'}</p>
                                            {selectedOrder.cancellation_info?.free_cancellation_before && (
                                                <p><strong>Free Cancellation Before:</strong> {formatDate(selectedOrder.cancellation_info.free_cancellation_before)}</p>
                                            )}
...
<CTableRow>
  ...
  <CTableDataCell>{selectedOrder.user_data?.user_comment || 'N/A'}</CTableDataCell>
</CTableRow>

<CTableRow>
  <CTableHeaderCell>Order Comment</CTableHeaderCell>
  <CTableDataCell>{selectedOrder.partner_data?.order_comment || 'N/A'}</CTableDataCell>
</CTableRow>
