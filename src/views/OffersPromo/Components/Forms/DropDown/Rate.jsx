    ...
    useEffect(() => {
        ...
        if (inventoryId && rate) {
            ...
            const processedOptions = rate.map((item) => ({
                origin_rate_id:  `${item.rate_id}-${item.inventory_id}`,
                label: `Currency: ${item.currency} MRP: ${item.mrp}, Purchase price: ${item.purchase_price}`,
            }));
            setOptions(processedOptions);
                ...
                category === "1" || category === "2" ? (
                    <>
                        ...
                        >
                            ...
                            {options.map((option) => (
                                <option key={option.origin_rate_id} value={option.origin_rate_id}>
                                    {option.label}
                                </option>
                            ))}
                        </CFormSelect>
