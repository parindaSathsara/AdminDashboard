  ...
  useEffect(() => {
    ...
    if(reteId && category === "5"){
      ...
      const processedOptions = filteredInventory.map((item) => ({
        discounted_inventory_id: item.inventory_id,
        ...
        category === "1" || category === "2" ? (
          <>
            ...
            >
              ...
              {options.map((option) => (
                <option key={option.discounted_inventory_id} value={option.discounted_inventory_id}>
                  {option.label}
                </option>
              ))}
            </CFormSelect>
           ...
           {label && <label htmlFor={id}>{label}</label>}
            <Select
              ...
              options={options.map((option) => ({
                value: option.discounted_inventory_id,
                label: option.label,
              }))}
              value={options.find((option) => option.discounted_inventory_id === value)}
